...
type Props = {
	...
	_cliff?: number;
	immutable?: boolean;
	tokenomicsBeneficiaries?: Beneficiary[];
	setTokenomicsBeneficiaries?: React.Dispatch<React.SetStateAction<Beneficiary[]>> | undefined;
};

function TokenomicsEntry({ beneficiaryId, _name, _percentage, _vestingFrequency, _vestingLength, _cliff, immutable, tokenomicsBeneficiaries, setTokenomicsBeneficiaries }: Props) {
	...
	return (
		<div className="flex flex-col space-y-2 bg-green-100 rounded text-sm p-4">
			<div className="flex justify-between items-center">
				<h3 className="text-lg font-semibold mb-2">Token Beneficiary {beneficiaryId}</h3>
				<XIcon
					onClick={() => {
						if (tokenomicsBeneficiaries && setTokenomicsBeneficiaries) {
							const newState = [...tokenomicsBeneficiaries];
							newState.splice(parseInt(beneficiaryId) - 1, 1);
							setTokenomicsBeneficiaries(newState);
						}
					}}
			...
			<div className="flex flex-wrap items-center text-left space-x-2">
				...
				<input
					...
					id={`tokenomics_beneficiary${beneficiaryId}_name`}
			...
			<div className="flex flex-wrap items-center text-left space-x-2">
				<p className="w-1/2 font-semibold">Get</p>
				<div className={`flex w-48 items-center px-4 border-[1px] border-gray-200 rounded-sm py-2 min-w-0 bg-white ${immutable && "bg-gray-200"}`}>
					<input
						...
						id={`tokenomics_beneficiary${beneficiaryId}_percentage`}
						className={`outline-none bg-transparent min-w-0 ${immutable && "bg-gray-200"}`}
						placeholder="% of funds raised"
						disabled={immutable}
						onWheel={(e) => (e.target as HTMLInputElement).blur()}
					/>
					<p className="text-gray-500 cursor-default ml-2 whitespace-nowrap">% tokens</p>
				</div>
			</div>
			...
			<div className="flex flex-wrap items-center text-left space-x-2">
				...
				<div className={`flex w-48 items-center px-4 border-[1px] border-gray-200 rounded-sm py-2 min-w-0 bg-white ${immutable && "bg-gray-200"}`}>
					<input
						...
						min={1}
						max={365}
						id={`tokenomics_beneficiary${beneficiaryId}_vesting_frequency`}
			...
			<div className="flex flex-wrap items-center text-left space-x-2">
				...
				<div className={`flex w-48 items-center px-4 border-[1px] border-gray-200 rounded-sm py-2 min-w-0 bg-white ${immutable && "bg-gray-200"}`}>
					<input
						...
						min={1}
						max={365}
						id={`tokenomics_beneficiary${beneficiaryId}_vesting_length`}
						className={`w-full outline-none bg-transparent min-w-0 ${immutable && "bg-gray-200"}`}
				...
				{/* <input type="text" value={vestingLength} onChange={(e) => setVestingLength(e.target.value)} id={`tokenomics_beneficiary${beneficiaryId}_vesting_length`} className={`flex justify-between items-center px-4 border-[1px] border-gray-200 rounded-sm py-2 min-w-0 ${immutable && "bg-gray-200"}`} placeholder="Vesting Length" disabled={immutable} /> */}
			</div>
			...
			<div className="flex flex-wrap items-center text-left space-x-2">
				...
				<div className={`flex w-48 items-center px-4 border-[1px] border-gray-200 rounded-sm py-2 min-w-0 bg-white ${immutable && "bg-gray-200"}`}>
					<input
						...
						min={1}
						max={365}
						id={`tokenomics_beneficiary${beneficiaryId}_cliff`}
						className={`w-full outline-none bg-transparent min-w-0 ${immutable && "bg-gray-200"}`}
			...
			<div className="flex flex-wrap items-center text-left space-x-2">
				...
				<input
					...
					id={`tokenomics_beneficiary${beneficiaryId}_address`}
...
export default TokenomicsEntry;
